...
export function createTestInteraction(
	...
	if (
		stats.totalAmountOfMirrors > 3 ||
		stats.totalAmountOfComments > 2 ||
	...
	switch (interaction.type) {
		case "MIRROR":
			if (!interaction.publicationId)
				throw new Error("Publication ID required for mirror");
		...
		case "LIKE":
			if (!interaction.publicationId)
				throw new Error("Publication ID required for like");
			return await client.like(interaction.publicationId);
		case "FOLLOW":
			if (!interaction.publicationId)
				throw new Error("Profile ID required for follow");
