...
export class LensClient {
	...
	async getPublication(pubId: string): Promise<AnyPublicationFragment | null> {
		...
		if (publication) this.cache.set(`lens/publication/${pubId}`, publication);
	...
	async getPublicationsFor(
		...
		do {
			...
			items.forEach((publication) => {
				this.cache.set(`lens/publication/${publication.id}`, publication);
			...
			do {
				...
				data.items.forEach((item) => {
					...
					if (timeline.length < limit && !item.root.isEncrypted) {
						this.cache.set(`lens/publication/${item.id}`, item.root);
