	...
	createClient: vi.fn(() => ({
		from: vi.fn(() => ({
			select: vi.fn(() => ({
				eq: vi.fn(() => ({
					maybeSingle: vi.fn(() =>
						Promise.resolve({ data: { id: "test-room-id" }, error: null }),
					),
				})),
			})),
		})),
...
describe("SupabaseDatabaseAdapter", () => {
	...
	describe("getRoom", () => {
		...
		it("should return null and log error when there is a database error", async () => {
			...
			expect(elizaLogger.error).toHaveBeenCalledWith(
				`Error getting room: ${error.message}`,
			);
		});
	});
	...
	describe("getParticipantsForAccount", () => {
		...
		it("should throw error when database error occurs", async () => {
			...
			await expect(adapter.getParticipantsForAccount(userId)).rejects.toThrow(
	...
	describe("setParticipantUserState", () => {
		...
		it("should throw error and log when database error occurs", async () => {
			...
			await expect(
				adapter.setParticipantUserState(roomId, userId, "FOLLOWED"),
			).rejects.toThrow("Failed to set participant user state");

			expect(elizaLogger.error).toHaveBeenCalledWith(
				"Error setting participant user state:",
				error,
			);
		});
