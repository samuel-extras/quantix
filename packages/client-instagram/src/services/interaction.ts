...
export class InstagramInteractionService {
		...
		const state = await this.runtime.composeState(
			{
				...
				roomId: stringToUuid(
					`instagram-temp-${Date.now()}-${this.runtime.agentId}`,
				),
	...
	private async handleComment(item: any) {
		try {
			...
			const roomId = stringToUuid(
				`instagram-comment-${item.media_id}-${this.runtime.agentId}`,
			);
			const commentId = stringToUuid(
				`instagram-comment-${comment.id}-${this.runtime.agentId}`,
			);
			const userId = stringToUuid(
				`instagram-user-${item.user_id}-${this.runtime.agentId}`,
			);
			...
			const state = await this.runtime.composeState(
				{
					userId: this.runtime.agentId,
					roomId: stringToUuid(
						`instagram-like-${item.media_id}-${this.runtime.agentId}`,
					),
					agentId: this.runtime.agentId,
	...
	private async handleMention(item: any) {
		try {
			const roomId = stringToUuid(
				`instagram-mention-${item.media_id}-${this.runtime.agentId}`,
			);
			const mentionId = stringToUuid(
				`instagram-mention-${item.pk}-${this.runtime.agentId}`,
			);
			const userId = stringToUuid(
				`instagram-user-${item.user.pk}-${this.runtime.agentId}`,
			);
	...
	private async createInteractionMemories(
		...
		await this.runtime.messageManager.createMemory({
			...
			createdAt: new Date(
				originalItem.timestamp || originalItem.created_at * 1000,
			).getTime(),
		});
		...
		await this.runtime.messageManager.createMemory({
			id: stringToUuid(
				`instagram-reply-${postedComment.id}-${this.runtime.agentId}`,
			),
			userId: this.runtime.agentId,
			agentId: this.runtime.agentId,
			content: {
