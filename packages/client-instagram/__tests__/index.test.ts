...
vi.mock("../src/services/interaction", () => ({
	InstagramInteractionService: vi
		.fn()
		.mockImplementation(() => mockInteractionService),
}));
...
describe("InstagramClientInterface", () => {
		...
		mockRuntime = {
			getSetting: vi.fn((key: string) => {
				if (
					...
					key === "INSTAGRAM_ENABLE_ACTION_PROCESSING"
				) {
	...
	it("starts in dry-run mode", async () => {
		...
		mockRuntime.getSetting = vi.fn((key: string) => {
			...
			if (key === "INSTAGRAM_ENABLE_ACTION_PROCESSING")
		...
		expect(result).toBeDefined();
		expect(elizaLogger.log).toHaveBeenCalledWith(
			"Instagram client running in dry-run mode",
		);
	...
	it("handles errors during startup", async () => {
		...
		await expect(InstagramClientInterface.start(mockRuntime)).rejects.toThrow(
			"Startup failed",
		);
		expect(elizaLogger.error).toHaveBeenCalledWith(
			"Failed to start Instagram client:",
			error,
		);
	});
	...
	it("stops gracefully", async () => {
		...
		expect(elizaLogger.log).toHaveBeenCalledWith(
			"Stopping Instagram client services...",
		);
	});
});
