...
export class MessageManager {
	...
	constructor(discordClient: any, voiceManager: VoiceManager) {
		...
		this.autoPostConfig = {
			enabled:
				this.runtime.character.clientConfig?.discord?.autoPost?.enabled ||
				false,
			monitorTime:
				this.runtime.character.clientConfig?.discord?.autoPost?.monitorTime ||
				300000,
			inactivityThreshold:
				this.runtime.character.clientConfig?.discord?.autoPost
					?.inactivityThreshold || 3600000, // 1 hour default
			mainChannelId:
				this.runtime.character.clientConfig?.discord?.autoPost?.mainChannelId,
	...
	async handleMessage(message: DiscordMessage) {
		...
		if (
			this.runtime.character.clientConfig?.discord?.shouldIgnoreBotMessages &&
			message.author?.bot
		) {
			return;
		}
			...
			if (
				...
				try {
					...
					let state = await this.runtime.composeState(memory, {
						...
						discordMessage: null,
						agentName:
							this.runtime.character.name || this.client.user?.displayName,
					});
					...
					const context = composeContext({
						state,
						template:
							this.runtime.character.templates?.discordAutoPostTemplate ||
							discordAutoPostTemplate,
					});
					...
					try {
						newsChannel
							...
							.on("collect", async (message: DiscordMessage) => {
								if (
									message.author.bot ||
									Date.now() - message.createdTimestamp > 300000
								)
									return;
								...
								try {
									...
									const roomId = stringToUuid(
										mainChannel.id + "-" + this.runtime.agentId,
									);
									const memory = {
	...
	private _checkInterest(channelId: string): boolean {
		...
		if (this._isTeamLeader() && channelState.messages.length > 0) {
			...
			if (
				!this._isRelevantToTeamMember(lastMessage.content.text || "", channelId)
			) {
				const recentTeamResponses = channelState.messages
					.slice(-3)
					.some(
						(m) =>
							m.userId !== this.client.user?.id && this._isTeamMember(m.userId),
					);
