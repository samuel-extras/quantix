...
class DeriveKeyProvider {
	...
	constructor(teeMode?: string) {
		...
		switch (teeMode) {
			...
			case TEEMode.PRODUCTION:
				endpoint = undefined;
				elizaLogger.log("TEE: Running in production mode without simulator");
				break;
			default:
				throw new Error(
	...
	async rawDeriveKey(
		...
		try {
			if (!path || !subject) {
				elizaLogger.error("Path and Subject are required for key derivation");
			}
	...
	get: async (runtime: IAgentRuntime, _message?: Memory, _state?: State) => {
		...
		try {
			...
			if (!runtime.getSetting("WALLET_SECRET_SALT")) {
				elizaLogger.error("Wallet secret salt is not configured in settings");
				return "";
			}
