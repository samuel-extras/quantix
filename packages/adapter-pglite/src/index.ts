	...
	async removeRoom(roomId: UUID): Promise<void> {
		...
		return this.withTransaction(async (tx) => {
			try {
				...
				await tx.query('DELETE FROM memories WHERE "roomId" = $1', [roomId]);
				await tx.query('DELETE FROM participants WHERE "roomId" = $1', [
					roomId,
				]);
				await tx.query('DELETE FROM goals WHERE "roomId" = $1', [roomId]);
	...
	async getKnowledge(params: {
		...
		return this.withDatabase(async () => {
			try {
				...
				const { rows } = await this.query<RAGKnowledgeItem>(sql, queryParams);
