...
const NVIDIA_NIM_NETWORKS = {
	...
	sandbox: {
		baseUrl:
			process.env.NVIDIA_NIM_SANDBOX_URL ||
...
export const nvidiaEnvSchema = z.object({
	...
	NVIDIA_NIM_LOG_LEVEL: z
		.enum(["error", "warn", "info", "debug"])
		.default("info"),
	NVIDIA_GRANULAR_LOG: z.boolean().default(false),
	NVIDIA_OFFTOPIC_SYSTEM: z
		.string()
		.default(
			"You are a topic control assistant. Your role is to analyze if the user's message is on-topic or off-topic based on the context and guidelines provided. Respond with a clear analysis of whether the message is appropriate for the given context.",
		),
	NVIDIA_OFFTOPIC_USER: z
		.string()
		.default("Please analyze if this message is on-topic or off-topic."),
...
export function getConfig(
	env: string | undefined | null = ENV || process.env.NVIDIA_NIM_ENV,
): NvidiaNimConfig {
	ENV = env || "production";
	...
	return {
		...
		NVIDIA_OFFTOPIC_SYSTEM:
			process.env.NVIDIA_OFFTOPIC_SYSTEM ||
			"You are a topic control assistant. Your role is to analyze if the user's message is on-topic or off-topic based on the context and guidelines provided. Respond with a clear analysis of whether the message is appropriate for the given context.",
		NVIDIA_OFFTOPIC_USER:
			process.env.NVIDIA_OFFTOPIC_USER ||
			"Please analyze if this message is on-topic or off-topic.",
	...
	try {
		...
		const config = {
			NVIDIA_NIM_ENV:
				...
				runtime.getSetting("NVIDIA_NIM_ENV") ||
				envConfig.NVIDIA_NIM_ENV,
			NVIDIA_NIM_SPASH:
				process.env.NVIDIA_NIM_SPASH ||
				runtime.getSetting("NVIDIA_NIM_SPASH") ||
				envConfig.NVIDIA_NIM_SPASH,
			NVIDIA_NIM_API_KEY:
				runtime.getSetting("NVIDIA_NIM_API_KEY") ||
				process.env.NVIDIA_NIM_API_KEY ||
				"",
			NVIDIA_NIM_MAX_RETRIES:
				process.env.NVIDIA_NIM_MAX_RETRIES ||
				runtime.getSetting("NVIDIA_NIM_MAX_RETRIES") ||
				envConfig.NVIDIA_NIM_MAX_RETRIES.toString(),
			NVIDIA_NIM_RETRY_DELAY:
				process.env.NVIDIA_NIM_RETRY_DELAY ||
				runtime.getSetting("NVIDIA_NIM_RETRY_DELAY") ||
				envConfig.NVIDIA_NIM_RETRY_DELAY.toString(),
			NVIDIA_NIM_TIMEOUT:
				process.env.NVIDIA_NIM_TIMEOUT ||
				runtime.getSetting("NVIDIA_NIM_TIMEOUT") ||
				envConfig.NVIDIA_NIM_TIMEOUT.toString(),
			NVIDIA_NIM_LOG_LEVEL:
				process.env.NVIDIA_NIM_LOG_LEVEL ||
				runtime.getSetting("NVIDIA_NIM_LOG_LEVEL") ||
				envConfig.NVIDIA_NIM_LOG_LEVEL,
			NVIDIA_GRANULAR_LOG:
				process.env.NVIDIA_GRANULAR_LOG === "true" ||
				runtime.getSetting("NVIDIA_GRANULAR_LOG") === "true" ||
				envConfig.NVIDIA_GRANULAR_LOG,
			NVIDIA_OFFTOPIC_SYSTEM:
				process.env.NVIDIA_OFFTOPIC_SYSTEM ||
				runtime.getSetting("NVIDIA_OFFTOPIC_SYSTEM") ||
				envConfig.NVIDIA_OFFTOPIC_SYSTEM,
			NVIDIA_OFFTOPIC_USER:
				process.env.NVIDIA_OFFTOPIC_USER ||
				runtime.getSetting("NVIDIA_OFFTOPIC_USER") ||
				envConfig.NVIDIA_OFFTOPIC_USER,
			NVIDIA_NIM_BASE_VISION_URL:
				process.env.NVIDIA_NIM_BASE_VISION_URL ||
				envConfig.NVIDIA_NIM_BASE_VISION_URL,
			NVIDIA_NIM_BASE_URL:
				process.env.NVIDIA_NIM_BASE_URL || envConfig.NVIDIA_NIM_BASE_URL,
			NVIDIA_COSMOS_INVOKE_URL:
				process.env.NVIDIA_COSMOS_INVOKE_URL ||
				envConfig.NVIDIA_COSMOS_INVOKE_URL,
			NVIDIA_COSMOS_ASSET_URL:
				process.env.NVIDIA_COSMOS_ASSET_URL ||
				envConfig.NVIDIA_COSMOS_ASSET_URL,
		};
		...
		throw new Error(
			`Failed to validate NVIDIA NIM configuration: ${errorMessage}`,
		);
	}
}
...
export const ALPHAFOLD2_CONFIG = {
	...
	API_URL:
		process.env.ALPHAFOLD_API_URL ||
		"https://health.api.nvidia.com/v1/biology/deepmind/alphafold2-multimer",
	STATUS_URL:
		process.env.ALPHAFOLD_STATUS_URL ||
		"https://health.api.nvidia.com/v1/status",
	SEQUENCES: {
