...
const invoke_url =
	config.NVIDIA_COSMOS_INVOKE_URL ||
	"https://ai.api.nvidia.com/v1/vlm/nvidia/cosmos-nemotron-34b";
const kNvcfAssetUrl =
	config.NVIDIA_COSMOS_ASSET_URL ||
...
export const getCosmosDescriptionAction: Action = {
	...
	similes: [
		"CHECK_COSMOS_DESCRIPTION",
		"ANALYZE_COSMOS_DESCRIPTION",
		"COSMOS_DESCRIPTION_CONTROL",
	],
	description:
		"Use NVIDIA Cosmos model to analyze and describe images or videos",
	examples: [
		[
			{
				user: "user",
			...
			} as ActionExample,
			{
				...
				content: {
					...
					data: {
						response:
							"The worker, who is wearing a neon vest, black pants, and a yellow hard hat, is pulling a yellow caution tape across the aisle. This action suggests that he is cordoning off the area, likely for safety or maintenance reasons.",
						analysis: {
							description:
								"The worker, who is wearing a neon vest, black pants, and a yellow hard hat, is pulling a yellow caution tape across the aisle. This action suggests that he is cordoning off the area, likely for safety or maintenance reasons.",
							confidence: 0.92,
	...
	handler: async (
		...
		try {
			...
			logGranular("API key configuration", {
				...
				source: config.NVIDIA_NIM_API_KEY
					? "config"
					: process.env.TEST_NVCF_API_KEY
						? "env"
						: "default",
			});
			...
			try {
				...
				if (parsedPrompt.isVideo) {
					...
					mediaPath = await assetManager.handleVideosCosmos(
						parsedPrompt.mediaFile,
					);
					...
					mediaPath = await assetManager.handleImagesCosmos(
						parsedPrompt.mediaFile,
					);
					...
					mediaPath = await assetManager.handleChatUploadCosmos(
						parsedPrompt.mediaFile,
						cosmosDir,
					);
				...
				const { data: authorizeRes } = await axios.post(
					kNvcfAssetUrl,
					postData,
					{ headers },
				);
