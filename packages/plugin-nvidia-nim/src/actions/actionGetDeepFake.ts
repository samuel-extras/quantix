...
export const getDeepFakeAction: Action = {
	...
	description:
		"Use NVIDIA DeepFake detection model to analyze images for potential manipulation",
	examples: [
		[
			{
				user: "user",
	...
	handler: async (
		...
		try {
			...
			if (parsedPrompt.isBase64) {
				...
				while (
					!fs.existsSync(path.join(workspaceRoot, "packages")) &&
					workspaceRoot !== path.parse(workspaceRoot).root
				) {
					workspaceRoot = path.dirname(workspaceRoot);
				}
				deepfakeDir = path.join(
					workspaceRoot,
					"packages",
					"plugin-nvidia-nim",
					"src",
				...
				if (!fs.existsSync(mediaPath)) {
					...
					try {
						const dirContents = await fs.promises.readdir(
			...
			try {
				...
				const faceDescriptions = analysis.bounding_boxes.map(
					(box, idx) =>
