...
export class AssetManager {
	...
	async downloadFromUrl(url: string): Promise<string> {
		...
		try {
			...
			const extension = Object.entries(SUPPORTED_FORMATS).find(
				([_, [mime]]) => mime === contentType,
			)?.[0];
	...
	async uploadAsset(
		filePath: string,
		description: string = "Reference media file",
	): Promise<MediaAsset> {
	...
	async handleImagesCosmos(uploadPath: string): Promise<string> {
		...
		const cosmosImageDir = path.join(
			workspaceRoot,
			"packages",
			"plugin-nvidia-nim",
			"src",
	...
	async handleVideosCosmos(uploadPath: string): Promise<string> {
		...
		const cosmosVideoDir = path.join(
			workspaceRoot,
			"packages",
			"plugin-nvidia-nim",
			"src",
