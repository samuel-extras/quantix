 ...
 */
export const parseCosmosPrompt = async (
	text?: string,
	attachments?: Media[],
	apiKey?: string,
): Promise<CosmosParsedPrompt> => {
	debugLog("Input", {
		hasText: !!text,
	...
	if (attachments?.length) {
		...
		if (apiKey) {
			...
			try {
				const newPath = await assetManager.handleChatUploadCosmos(
					uploadPath,
					cosmosDir,
				);
 ...
 */
export function createCosmosPrompt(
	mediaFile: string,
	query: string = "Describe the scene",
): string {
	return `[MEDIA]
${mediaFile}
[/MEDIA]
