...
describe("SQLite3VerifiableDAO", () => {
	...
	beforeEach(() => {
		...
		verifiableLogProvider = new VerifiableLogProvider(
			sqLite3VerifiableDAO,
			"LOCAL",
		);
	});
	describe("VerifiableLogProvider Management", () => {
		...
		it("should registerAgent and  getAgent when available", async () => {
			...
			const stringPromise = await verifiableLogProvider.generateAttestation({
				agentId: testAgentId,
				publicKey: pageResult1.agent_keypair_vlog_pk,
			});
