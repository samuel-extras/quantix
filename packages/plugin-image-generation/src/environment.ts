...
export async function validateImageGenConfig(
	...
	try {
		const config = {
			...
			TOGETHER_API_KEY:
				runtime.getSetting("TOGETHER_API_KEY") || process.env.TOGETHER_API_KEY,
			HEURIST_API_KEY:
				runtime.getSetting("HEURIST_API_KEY") || process.env.HEURIST_API_KEY,
			...
			VENICE_API_KEY:
				runtime.getSetting("VENICE_API_KEY") || process.env.VENICE_API_KEY,
