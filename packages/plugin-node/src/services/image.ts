...
class GroqImageProvider implements ImageProvider {
	...
	async describeImage(
		...
		const response = await fetch(endpoint + "/chat/completions", {
			...
			body: JSON.stringify({
				model:
					/*this.runtime.imageVisionModelName ||*/ "llama-3.2-90b-vision-preview",
				messages: [{ role: "user", content }],
				max_tokens: 1024,
			}),
		});
